...
const useCoinflip = (betNum, betAmount) => {
  const { getMaxPayout, getBetHistory, approveToken, refreshPages, playCoinflip, decodeTxLogs, A2D } = useContract()
  ...
  useEffect(() => {
    ...
    if (contracts && contracts.coinflip) {
      ...
      getBetHistory(contracts.coinflip)
      .then((r) => {
        if (ac.signal.aborted === false) {
          setBetHistory(r)
        }
    ...
    return () => ac.abort()
  }, [betNum, getMaxPayout, getBetHistory, contracts.coinflip])
  ...
  const play = useCallback(() => {
    ...
    const id = toast.loading(`Casting a Coin for ${betNum === 1 ? "tails" : "heads"}...`, toastOptions)
